...
#
# Submit 20-seed sweep for TFIM 4q noiseless
# Usage: bash scripts/submit_sweep_tfim_4q.sh
...
CONFIGS_DIR="${PROJECT_ROOT}/configs/seed_sweep/tfim_4q"
...
for cfg in ${CONFIGS_DIR}/*.yaml; do
    ...
    JOB_ID=$(sbatch --parsable \
        ...
        --ntasks-per-node=1 \
        --cpus-per-task=56 \
        --output=slurm_logs/tfim_4q_seed${seed}.o%j \
        --error=slurm_logs/tfim_4q_seed${seed}.e%j \
...
echo ""
echo "Submitted ${JOB_COUNT} jobs for TFIM 4q"
