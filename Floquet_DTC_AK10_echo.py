...
def build_autocorr_circuit(num_kicks, hz_angles, Jzz_angles, init_bit_array):
...
def build_echo_circuit(num_kicks, hz_angles, Jzz_angles, init_bit_array):
    ...
    for _ in range(num_kicks):
        apply_one_Floquet_period_conjugate(qc, hz_angles, Jzz_angles)

    qc.measure(range(num_qubits), range(num_qubits))

    return qc

def build_echo_instance_and_run(Jz_angles, hz_angles, init_bit_array):
    echo_values = []
    
    echo_circuits = [
        build_echo_circuit(n_kicks, hz_angles, Jz_angles, init_bit_array)
...
def build_autocorr_instance_and_run(Jz_angles, hz_angles, init_bit_array):
    autocorrelators = []
    circuits = [
        build_autocorr_circuit(n_kicks, hz_angles, Jz_angles, init_bit_array)
        for n_kicks in range(num_max_kicks)
    ]
...
def run_autocorr_and_echo(Jz_angles, hz_angles, init_bit_array):
    autocorrelators = build_autocorr_instance_and_run(Jz_angles, hz_angles, init_bit_array)
    echo_values = build_echo_instance_and_run(Jz_angles, hz_angles, init_bit_array)
...
def main():
    total_autocorrelators = np.zeros(num_max_kicks)
    total_echo_values = np.zeros(num_max_kicks)
    ...
    for n in range(num_gate_instances):
        ...
        if Jz_frozen == True: # overrides randomized Jz:s
        ...
        # single call for both for simple parallelization 
        # autocorr and echo can be done seaparately also (and leave echo away if needed)
        temp_autocorrelators, temp_echo_values = run_autocorr_and_echo(Jz_angles, hz_angles, init_bit_array)

        for n in range(num_max_kicks):
            total_autocorrelators[n] += temp_autocorrelators[n] 
            total_echo_values[n] += temp_echo_values[n] 

    normal_autocorrelators = np.array(total_autocorrelators) / num_gate_instances
    normal_echo_values = np.array(total_echo_values) / num_gate_instances
    
    end_time = time.time()
    print(f"Run time for {num_gate_instances} gate instances: {end_time-start_time}")
    autocorr_data_to_file(normal_autocorrelators)
    echo_data_to_file(normal_echo_values)
    plot_results(normal_autocorrelators, normal_echo_values)
